...
## 2026-08-02 外部挂单不再计入超时失败

- 外部挂单的数量差异属于状态信息，不属于程序调价失败；完整运行现在不会将其加入 `errors`。
- 外部挂单仍会在当前在售列表中显示数量明细；程序管理挂单的调价异常仍会正常报告。
- 验证：全量测试 `65 passed`，相关 Ruff、前端语法检查和 `git diff --check` 均通过。

## 2026-08-03 价格同步失败时阻止完整运行上架

- 价格同步返回错误时，完整运行逐项记录 `价格同步：物品：原因`，停止生成新计划，并阻止第 4 步提交上架。
- 价格同步没有报错但本地仍没有可用价格历史时，同样逐项报告价格数据缺失并停止本次上架，避免静默跳过物品。
- 已有在售挂单不受影响；仅阻止本次完整运行中新计划的提交。
- 新增价格同步失败后不生成计划、不提交上架的回归测试。
- 验证：全量测试 `66 passed`，相关 Ruff、前端语法检查和 `git diff --check` 均通过。
